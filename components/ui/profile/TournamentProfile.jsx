...
export default function TournamentProfile({ navigation, route = { params: {} } }) {
  ...
  */
    ...
    try {
      const [tournRes, matchesRes, pointsRes, batLeadRes, bowlLeadRes] =
        await Promise.allSettled([
          ...
          (async () => {
            const r1 = await request(`api/tournaments/leaderboard/${tournamentId}?type=bowling`).catch(() => null);
            const s1 = r1?.data?.stats || r1?.stats || r1?.data;
            if (Array.isArray(s1) && s1.length > 0) return r1;
            return request(`api/tournaments/leaderboard/${tournamentId}?type=fielding`).catch(() => null);
          })(),
        ]);
  ...
  const toDisplayNumber = (value, fallback = 0) => {
    const raw = typeof value === "object" ? toDisplayText(value, "") : value;
    const parsed = Number(raw);
    return Number.isFinite(parsed) ? parsed : fallback;
  };

  const formatStatNumber = (value, fallback = "0.00") => {
    const parsed = toDisplayNumber(value, NaN);
  ...
  const renderLeaderboardItem = (item, index, type) => {
    ...
    const teamName =
      ...
      toDisplayText(item.teamId) ||
      "";
    const wickets = toDisplayNumber(
      item.wickets ?? item.totalWickets ?? item.wicketsTaken ?? item.wicket,
      0
    );
    const runs = toDisplayNumber(item.runs ?? item.totalRuns, 0);
    const innings = toDisplayNumber(
      item.matchesPlayed ?? item.innings ?? item.matches ?? item.totalMatches,
      0
    );
    const average = toDisplayText(item.battingAverage ?? item.average ?? item.avg, "0.00");
    const strikeRate = toDisplayText(item.strikeRate ?? item.sr, "0.00");
    const economy = formatStatNumber(item.economyRate ?? item.economy ?? item.econ);
    const profileImage = item.profileImg || item.player?.profileImg || item.playerId?.profileImg;
    const primaryStat =
      type === "batting"
        ? `${runs} runs`
        : `${wickets} wkts`;
    ...
    return (
      <View
        key={toDisplayText(item.id || item._id || item.playerId || item.player, `${type}_${index}`)}
      ...
      >
        <View className="flex-row justify-between items-center">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            {profileImage ? (
              <Image
                source={{ uri: getImageFullUrl(profileImage) }}
